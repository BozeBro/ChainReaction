...
class chainReaction {
  ...
  is_side(x, y) {
    return ( (0 < x && x < this.cols - 1) && (y == 0 || y == this.rows - 1) ) 
      || ( (x == 0 || x == this.cols - 1) && (0 < y && y < this.rows - 1) );
  }
  is_explode(x, y) {
    let sq = this.board[y][x];
    return (this.is_corner(x, y) && sq["circles"] >= 2) || ( this.is_side(x,y) && sq["circles"] >= 3 ) || ( sq["circles"] >= 4 );   
  }
  get_neighbors(x, y) {
    let ans = []
    for ( const [dx, dy] of [[1,0], [0,1],[-1,0], [0,-1]] ) {
      let nx = x + dx;
      let ny = y + dy;
      if (0 <= nx && nx < this.cols && 0 <= ny && ny < this.rows) {
        ans.push([nx, ny]);
      }
    }
    return ans;
  ...
  explode(x, y, color) {
    ...
    while (que.length > 0) {
      ...
      while (que.length > 0) {
        let sq = que.shift();
        let neighbors = this.get_neighbors(sq.x, sq.y);
        this.board[sq.y][sq.x]["color"] = ""
        this.board[sq.y][sq.x]["circles"] = 0;
        for (const [nx, ny] of neighbors) {
          this.board[ny][nx]["circles"]++;
          this.board[ny][nx]["color"] = color;
          if (this.is_explode(nx, ny)) {
            nxt.push({x: nx, y: ny});
          }
        }
      }
  ...
  move(x, y, color) {
    ...
    this.board[y][x]["circles"]++;
    if ( this.is_explode(x, y) ) {
      console.log("we going to explode");
  ...
  initBoard() {
    ...
    for (let c = 0; c < this.cols; c++) {
      ...
      for (let r = 0; r < this.rows; r++) {
        this.board[c][r] = {color: "", circles: 0};
      }
    }
  }
  ...
  animate(animations, unmoving, ts, start, ind, data) {
    ...
    if (elapsed < this.__ms) {
      ...
      if (this.que.length > 0) {
        let info = this.que[0]
        this.msgHandler[info.type.toLowerCase()](info)
      }
